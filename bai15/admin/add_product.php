...
if(isset($_POST['sbm'])){
    ...
    $prd_image_type = $_FILES['prd_image']['type'];
    $prd_image_tmp_name = $_FILES['prd_image']['tmp_name'];
    $prd_image_error = $_FILES['prd_image']['error'];
    $prd_image_size = $_FILES['prd_image']['size'];
    $str =pathinfo($_FILES['prd_image']['name'], PATHINFO_EXTENSION);
    ...
    if(in_array($str, $typeFileAllow)){
        $error = "File bạn vừa chọn hệ thống không hỗ trợ, chỉ tải được file có định dạng 'png','jpg','jpeg', gif";
        // echo $error;
